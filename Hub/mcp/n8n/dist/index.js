...
// ── list_workflows ─────────────────────────────────────────────────────────
server.registerTool("n8n_list_workflows", { description: "List all workflows in N8N. Returns id, name, active status and tags." }, async () => {
    const res = await api.get("/workflows");
    const workflows = res.data.data.map((w) => ({
        id: w.id,
        name: w.name,
        active: w.active,
...
server.registerTool("n8n_create_workflow", {
    description: "Create a new workflow in N8N. Pass the full workflow JSON object (name, nodes, connections, settings).",
    inputSchema: {
    ...
    return {
        content: [
            {
                type: "text",
                text: JSON.stringify({ id: res.data.id, name: res.data.name, active: res.data.active }, null, 2),
            },
        ],
    };
});
...
// ── get_executions ─────────────────────────────────────────────────────────
server.registerTool("n8n_get_executions", {
