...
const server = new Server(
  { name: 'ledgerops-monday', version: '1.0.0' },
  { capabilities: { tools: {} } }
);
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    if (name === 'monday_find_board_by_name') {
      const nameFilter = (args?.name as string).toLowerCase();
      const data = await mondayQuery(`
        query { boards(limit: 50) { id name } }
      `) as { boards: { id: string; name: string }[] };
    ...
    if (name === 'monday_create_item') {
      const { board_id, item_name, group_id, column_values } = args as {
        board_id: string; item_name: string; group_id?: string; column_values?: string;
      };
    ...
    if (name === 'monday_create_subitem') {
      const { parent_item_id, subitem_name, column_values } = args as {
        parent_item_id: string; subitem_name: string; column_values?: string;
      };
      const colPart = column_values ? `, column_values: ${JSON.stringify(column_values)}` : '';
      const data = await mondayQuery(`
        mutation {
      ...
      `) as { create_subitem: unknown };
      return { content: [{ type: 'text', text: JSON.stringify(data.create_subitem, null, 2) }] };
    }

    if (name === 'monday_update_item') {
      const { board_id, item_id, column_values } = args as {
        board_id: string; item_id: string; column_values: string;
      };
      const data = await mondayQuery(`
        mutation {
    ...
    if (name === 'monday_get_users') {
      ...
      `) as { users: { id: string; name: string; email: string }[] };
      const result = email
